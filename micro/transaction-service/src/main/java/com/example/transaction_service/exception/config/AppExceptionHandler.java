package com.example.transaction_service.exception.config;

import com.example.transaction_service.exception.UnauthorizedException;
...
public class AppExceptionHandler {
     ...
     * @return {@link ErrorStruct}, {@link HttpStatus#BAD_REQUEST}
     *
     */
    @ExceptionHandler(value = IllegalArgumentException.class)
    public ResponseEntity<?> handleIllegalArgumentException(IllegalArgumentException exception){
        log.error(exception.getMessage(),exception);
        var errorStruct = ErrorStruct.withDefaultLocalDateTime(HttpStatus.BAD_REQUEST,exception.getMessage()).build();
        return new ResponseEntity<>(errorStruct,errorStruct.getHttpStatus());
    }
